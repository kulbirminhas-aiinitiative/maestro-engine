"""
Gateway Client SDK for Maestro Engine

Provides interface for communicating with other services through MAESTRO API Gateway.
...
Usage:
    ...
    # Call quality-fabric
    response = await gateway_client.call("quality", "/api/validate", method="POST", json={...})

    # Synchronous version
    response = gateway_client.call_sync("templates", "/api/search", method="POST", json={...})
"""
...
class GatewayClient:
    """
    Client for inter-service communication through MAESTRO API Gateway
    ...
    def __init__(
        ...
        self.service_name = service_name or os.getenv("SERVICE_NAME", "maestro-engine")
        ...
        logger.info(
            f"Gateway client initialized: gateway_url={self.gateway_url}, service_name={self.service_name}"
        )
    ...
    def _build_url(self, service: str, path: str) -> str:
        ...
        Args:
            service: Target service name (e.g., "templates", "quality", "rag")
    ...
    async def call(
        ...
        logger.debug(f"Gateway call: {method} {service}{path}")
        ...
        try:
            ...
            logger.info(f"Gateway call success: {method} {service}{path} -> {response.status_code}")

            return response

        except httpx.TimeoutException as e:
            logger.error(f"Gateway call timeout: {method} {service}{path} - {str(e)}")
            raise

        except httpx.HTTPError as e:
            logger.error(f"Gateway call error: {method} {service}{path} - {str(e)}")
            raise
    ...
    def call_sync(
        ...
        Example:
            response = gateway_client.call_sync(
                "templates",
                "/api/search",
                method="POST",
                json={"query": "auth"}
        ...
        logger.debug(f"Gateway call (sync): {method} {service}{path}")
        ...
        try:
            ...
            logger.info(
                f"Gateway call success (sync): {method} {service}{path} -> {response.status_code}"
            )

            return response

        except httpx.TimeoutException as e:
            logger.error(f"Gateway call timeout (sync): {method} {service}{path} - {str(e)}")
            raise

        except httpx.HTTPError as e:
            logger.error(f"Gateway call error (sync): {method} {service}{path} - {str(e)}")
            raise
...
# Singleton instance for maestro-engine
gateway_client = GatewayClient(service_name="maestro-engine")
