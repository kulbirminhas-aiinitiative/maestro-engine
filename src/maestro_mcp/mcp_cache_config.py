...
class MCPCacheManager:
        ...
        try:
            ...
            session_state = {
                ...
                "active_sessions": list(
                    set(entry.get("session_id", "unknown") for entry in self.session_cache.values())
                ),
            }
    ...
    def store_persona_analysis(
        self, persona_name: str, requirement: str, analysis: Dict[str, Any], session_id: str
    ) -> str:
    ...
    def store_generated_artifacts(
        self, persona_name: str, artifacts: Dict[str, Any], session_id: str
    ) -> str:
        """Store generated artifacts (files, configs, docs) in MCP cache"""

        cache_key = self._generate_cache_key(
            "artifacts", f"{session_id}_{int(time.time())}", persona_name
        )
        ...
        cache_entry = {
            ...
            "created_at": datetime.now().isoformat(),
            "expires_at": (
                datetime.now() + timedelta(hours=self.ttl_hours * 2)
            ).isoformat(),  # Artifacts last longer
            "persona_source": persona_name,
            "session_id": session_id,
            "metadata": {
                "artifact_types": list(artifacts.keys()),
                "total_files": sum(
                    len(v) if isinstance(v, list) else 1 for v in artifacts.values()
                ),
    ...
    def get_previous_persona_work(
        self, session_id: str, exclude_persona: str = ""
    ) -> Dict[str, Any]:
        ...
        for key, entry in self.session_cache.items():
            if (
                ...
                and entry.get("persona_source") != exclude_persona
                and self._is_entry_valid(entry)
            ):
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "personas_active": len(
                set(entry.get("persona_source", "unknown") for entry in self.session_cache.values())
            ),
            "cache_hit_data": {
...
if __name__ == "__main__":
    ...
    key = cache.store_persona_analysis(
        "test_persona", "test requirement", test_analysis, "test_session_123"
    )
